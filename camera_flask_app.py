    ...
    while True:
        ...
        if success:
            ...
            if (capture):
                ...
                p = os.path.sep.join(['data', "test.jpg"])
                cv2.imwrite(p, frame)
...
def tasks():
    ...
    if request.method == 'POST':
        if request.form.get('click') == 'Capture':
            ...
            global neg
            neg = not neg
        elif request.form.get('Recognition') == 'Recognition':
            import pipeline
            pipeline.start_process()
            import demo
            demo.run()
            import write_text
            write_text.start_write()
        elif request.form.get('face') == 'Word Focus':
            global face
            face = not face
            if (face):
                time.sleep(4)
            ...
            if (switch == 1):
                ...
                camera = cv2.VideoCapture(0)
                switch = 1
        elif request.form.get('rec') == 'Recording/End':
            global rec, out
            rec = not rec
            if (rec):
                now = datetime.datetime.now()
